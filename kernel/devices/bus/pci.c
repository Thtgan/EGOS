...
PCIdevice* pci_getDevice(Index32 index) {
    ...
    ERROR_THROW_NEW_IF(ret == OBJECT_NULL, ERROR_INVALID_ARGUMENT, error_out);
...
Uint32 pci_getBARaddr(Uint32 baseAddr, Uint8 barIndex) {
    ERROR_THROW_NEW_IF(barIndex > 5, ERROR_INVALID_ARGUMENT, error_out);
...
Uint32 pci_readBAR(Uint32 baseAddr, Uint8 barIndex) {
    ...
    ERROR_THROW_NEW_IF(addr == (Uint32)-1, ERROR_INVALID_STATE, error_out);
...
void pciMSIX_initStruct(pciMSIX* msix, Uint32 baseAddr) {
    ...
    ERROR_THROW_NEW_IF(msix->offset == 0 || (msix->offset & 3) != 0, ERROR_IO_FAILED, error_out);
...
void pciMSIX_addEntry(pciMSIX* msix, Index16 msixVec, int cpuID, int interruptVec, bool edgetrigger, bool deassert) {
    ERROR_THROW_NEW_IF(msixVec >= msix->capacity, ERROR_INVALID_ARGUMENT, error_out);
...
static void __pci_readDevice(Uint32 baseAddr, PCIdevice* device) {
    ...
    ERROR_THROW_NEW_IF(vendorID == PCI_COMMON_HEADER_INVALID_VENDOR_ID, ERROR_INVALID_STATE, error_out);

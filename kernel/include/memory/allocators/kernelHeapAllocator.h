...
typedef struct KernelHeapSubAllocator KernelHeapSubAllocator;
...
typedef struct KernelHeapSubAllocator {
    HeapAllocator       allocator;
    SinglyLinkedList    list;
    Size                remaining;
} KernelHeapSubAllocator;
...
#define KERNEL_HEAP_ALLOCATOR_ORDER_NUM                 (HEAP_ALLOCATOR_MAXIMUM_ACTUAL_SIZE_SHIFT - KERNEL_HEAP_ALLOCATOR_MIN_ACTUAL_SIZE_SHIFT + 1)    //Orcer 0(16B) -> Order8(4096B)
...
typedef struct KernelHeapAllocator {
    ...
    KernelHeapSubAllocator  subAllocators[KERNEL_HEAP_ALLOCATOR_ORDER_NUM];
} KernelHeapAllocator;

...
static void __memoryOperations_fileCopyEntry(PagingLevel level, ExtendedPageTable* srcExtendedTable, ExtendedPageTable* desExtendedTable, Index16 index);
...
static void __memoryOperations_fileFaultHandler(PagingLevel level, ExtendedPageTable* extendedTable, Index16 index, void* v, HandlerStackFrame* handlerStackFrame, Registers* regs);
...
static void* __memoryOperations_fileReleaseEntry(PagingLevel level, ExtendedPageTable* extendedTable, Index16 index);
...
static MemoryOperations __memoryOperations_defaultOperations[DEFAULT_MEMORY_OPERATIONS_TYPE_NUM] = {
    ...
    [DEFAULT_MEMORY_OPERATIONS_TYPE_FILE] = (MemoryOperations) {
        .copyPagingEntry = __memoryOperations_fileCopyEntry,
        .pageFaultHandler = __memoryOperations_fileFaultHandler,
        .releasePagingEntry = __memoryOperations_fileReleaseEntry
    }
};
    ...
    if (PAGING_IS_LEAF(level, *srcEntry)) {
        ...
        *desExtraEntry = *srcExtraEntry;
        ...
        for (int i = 0; i < PAGING_TABLE_SIZE; ++i) {
            ...
            __memoryOperations_fileCopyEntry(PAGING_NEXT_LEVEL(level), srcSubExtendedTable, desSubExtendedTable, i);
            ERROR_GOTO_IF_ERROR(0);
        }
...
static void* __memoryOperations_fileReleaseEntry(PagingLevel level, ExtendedPageTable* extendedTable, Index16 index) {
    ...
    if (PAGING_IS_LEAF(level, *entry)) {
        ...
        for (int i = 0; i < PAGING_TABLE_SIZE; ++i) {
            ...
            DEBUG_ASSERT_SILENT(subExtendedTable->extraTable.tableEntries[i].operationsID == DEFAULT_MEMORY_OPERATIONS_TYPE_ANON);
            __memoryOperations_fileReleaseEntry(PAGING_NEXT_LEVEL(level), subExtendedTable, i);
            ERROR_GOTO_IF_ERROR(0);
        }
        extendedPageTable_freeFrame(PAGING_CONVERT_KERNEL_MEMORY_V2P(subExtendedTable));
    }
